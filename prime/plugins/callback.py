...
from prime import YouTube, app, userbot
from prime.core.call import assistantcall
from prime.misc import SUDOERS, db
from prime.utils import bot_sys_stats
from prime.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from prime.utils.database.memorydatabase import get_upvote_count
from prime.utils.decorators.language import languageCB
from prime.utils.formatters import seconds_to_min
from prime.utils.inline import (
...
from prime.utils.stream.autoclear import auto_clean
from prime.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await assistantcall.pause_stream(chat_id)
        ...
        await assistantcall.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await CallbackQuery.answer()
        await assistantcall.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await assistantcall.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await assistantcall.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                ...
                image = None
            try:
                await assistantcall.skip_stream(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await assistantcall.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
        ...
        try:
            await assistantcall.seek_stream(
                chat_id,
                file_path,
